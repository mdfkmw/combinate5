...
router.get('/:scheduleId/default-vehicle', async (req, res) => {
  try {
...
router.put('/:scheduleId/default-vehicle', async (req, res) => {
...
router.get('/:scheduleId/seat-blocks', async (req, res) => {
  try {
    const schedule = await loadSchedule(req.params.scheduleId);
    if (!schedule) {
      return res.status(404).json({ error: 'Programarea nu există.' });
...
router.put('/:scheduleId/seat-blocks', async (req, res) => {
  const scheduleId = Number(req.params.scheduleId);
